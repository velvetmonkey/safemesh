...
Record sequences start at 1. Raw `OrSet` refuses an add record at sequence 0 with `WireError::ZeroSequenceAdd`, on every admission path (`EventLog::admit_with`, `insert_record`, `merge_records`) and in the checked log loaders (`from_wire_bytes_for`, `records_from_wire_bytes_for`). This follows the ownership rule in [`RecordKernel.lean`](https://github.com/velvetmonkey/safemesh/blob/main/lean/SafeMesh/RecordKernel.lean): `allocateToken` only allocates at a positive sequence, and an add's payload must match that allocation. The raw carrier has no writer count, so it checks only the sequence; the author bound and the token value stay with the checked writers. A stored log that already holds such a record fails to load, and the error names the record's author. To recover, decode the log with the inert `EventLog::from_wire_bytes`, drop the sequence-0 add, re-add its element from the same author at a positive sequence, and write the log again.
