...
Record sequences start at 1. Raw `OrSet` refuses sequence-0 adds with `WireError::ZeroSequenceAdd` and sequence-0 removes with `WireError::ZeroSequenceRemove`, on every admission path (`EventLog::admit_with`, `insert_record`, `merge_records`) and in the checked log loaders (`from_wire_bytes_for`, `records_from_wire_bytes_for`). This follows [`RecordKernel.permitted`](https://github.com/velvetmonkey/safemesh/blob/main/lean/SafeMesh/RecordKernel.lean), which requires a positive sequence for every payload. An add also requires a token allocated at a positive sequence. The raw carrier has no writer count, so it checks only the sequence; the author bound and add token value stay with the checked writers. A stored log holding either record fails to load, and the error names its author. To recover, decode the log with the inert `EventLog::from_wire_bytes`, drop the sequence-0 record, issue the add or remove again from the same author at a positive sequence, and write the log again.
