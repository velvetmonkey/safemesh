...
- `rust/crates/safemesh-crdt` — API present: `no_std + alloc` G-Set, delta G-Counter, delta PN-Counter, delta OR-Set, and RGA/Text state. Build checked: CI cross-compiles the core for `thumbv7em-none-eabihf` on Linux. Runtime tested: `tests/conformance.rs` replays inputs from the Lean-generated JSON corpus (`lake exe corpus`) through the host Rust implementation and compares state vectors, sets, read vectors, numeric values, ownership decisions and optional allocation/sequence results with the parsed expectations, including permutations and redeliveries. Hardware runtime evidence is not provided by that cross-build.
