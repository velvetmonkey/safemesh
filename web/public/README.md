...
Lean is the semantic oracle. G-Counter mirrors Lean semantics that also ship in Rust and are differentially tested over corpus C. OR-Set mirrors Lean-proven add-wins semantics directly; Rust OR-Set now ships in `safemesh-crdt` and is differentially tested over corpus C.
