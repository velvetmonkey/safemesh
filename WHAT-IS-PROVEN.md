...
4. Rust product code replays that corpus and must match it byte-for-byte. **TESTED** — Evidence: [`conformance`](rust/crates/safemesh-crdt/tests/conformance.rs); run from `rust/`: `cargo test -p safemesh-crdt --test conformance --locked`.
