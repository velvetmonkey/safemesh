...
def identity(row):
    """Use recorded identity, or explicit identity words in a legacy assertion.

    A family number, operation position, or zero write count cannot identify a
...
def measurement_label(row):
    return '; '.join((*identity(row), row['op']))
...
def generate(raw):
    ...
    for family in sorted({h['family'] for _, h, _ in groups} | set(stopmap)):
        ...
        lines += [f"FAMILY {family} RAN yes; MEASUREMENTS {', '.join(sorted({measurement_label(r) for r in rows if 'ns' in r}))}; RUNGS retained setup records {', '.join(str(x[1]['n']) for x in selected)}; STOPPED BY box brake: {stopmap.get(family,'completion')}; REPETITIONS 7 per condition. Top complete rung {meta['name']}: payload {head['payload']} bytes, actual writer domain {head['writers'] if family in ['B1','B2','B3','B4'] else 1}.", '', '| Operation / condition | p50/p95/p99 ms | Peak RSS bytes | Peak allocator bytes | Cumulative allocation bytes, min–max | Kernel write bytes, min–max |', '|---|---:|---:|---:|---:|---:|']
        keys=sorted({(measurement_label(r),r.get('condition','warm'),str(r.get('duplicate_percent_requested',r.get('kind','')))) for r in rows if 'ns' in r})
        for op,condition,kind in keys:
            rs=[r for r in rows if 'op' in r and measurement_label(r)==op and r.get('condition','warm')==condition and str(r.get('duplicate_percent_requested',r.get('kind','')))==kind]
            lines += [f"| {measurement_label(rs[0])} / {condition} {kind} | {distribution(rs)} | {max(r['process_peak_rss_bytes'] for r in rs)} | {max(r['allocator_peak_bytes'] for r in rs)} | {min(r['allocation_bytes'] for r in rs)}–{max(r['allocation_bytes'] for r in rs)} | {min(r['kernel_write_bytes'] for r in rs)}–{max(r['kernel_write_bytes'] for r in rs)} |"]
        if family=='B1':
        ...
        lines+=['']
    measured = sorted({identity(r)[0] for _, _, rows in groups for r in rows if 'ns' in r})
    lines += [f"Carriers recorded in completed rows: {', '.join(measured) or 'none'}. Missing CRDT or durability fields are reported as not recorded; no carrier coverage is inferred from family IDs. Legacy assertions supply identity only when they explicitly name it. Timings cannot be extrapolated to other carriers or durability modes.", '', '## T3/T4 — Eight measured dimensions and candidate proposal', '', '| Dimension | Min | Max reached | Top measurement | Stop | Peak RSS / allocator bytes | p50/p95/p99 ms | Write amplification |', '|---|---:|---:|---|---|---:|---:|---|']
    ...
    for dim,items in candidates.items():
        ...
        rs=[r for value,name,family,r in items if value==maximum and name==top[1] and measurement_label(r)==measurement_label(top[3]) and r.get('condition')==top[3].get('condition')]
        lines.append(f"| {dim} | {minimum} | {maximum} | {top[1]} {measurement_label(top[3])} {top[3].get('condition','warm')} (n={len(rs)}) | {stopmap.get(top[2],'completion')} | {max(r['process_peak_rss_bytes'] for r in rs)} / {max(r['allocator_peak_bytes'] for r in rs)} | {distribution(rs)} | {amplification(rs)} |")
...
def controls(parent):
    ...
    for name,rs in base+planted:
        ...
        lines += [f"| {name} {measurement_label(rs[0])} | {distribution(rs)} | {rs[0]['allocation_bytes']} | {rs[0]['allocator_extra_peak_bytes']} |"]
