...
part of the nine retained bootstrap files or their generating SHA below.
...
archive the exact claimed commit into an isolated source tree. They copy this
checkout's generator into that tree, regenerate the nine fixtures, and compare
each file byte for byte. `BOOTFIXTURE_SOURCE_SHA` and
`BOOTFIXTURE_RETAINED_DIR` are for negative controls; the normal run uses the
pinned SHA and retained directory.

```sh
set -eu
source_sha="${BOOTFIXTURE_SOURCE_SHA:-3a9179b0cd98ae7e5c6803a6de014416320929ba}"
claimed_sha="$(sed -n 's/^Generating source: `\([0-9a-f]*\)`\.$/\1/p' rust/crates/safemesh-crdt/tests/fixtures/bootstrap/README.md)"
...
cp rust/crates/safemesh-crdt/tests/bootstrap_generate.rs "$scratch/baseline/rust/crates/safemesh-crdt/tests/bootstrap_generate.rs"
...
for name in counter.log counter.transaction counter.fence orset.log orset.transaction orset.fence pn.log gset.state rga.state; do
...
Do not regenerate these retained bytes with a newer source. Replay: `cargo test
