...
|---|---|---|---|
| [Interactive web convergence hero](web-hero/README.md) | `cd web && npm install && npm run dev` | Scenario buttons for normal delivery, duplicate replay, out-of-order delivery, dropped-message recovery, and partition heal, all ending at the same modeled state. | TypeScript is a demo mirror, not the verified artifact. |
